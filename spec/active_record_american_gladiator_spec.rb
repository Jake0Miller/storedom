...
describe "ActiveRecord American Gladiator" do
  ...
  context "Powerball" do
    it "returns all items containing Powerball" do
      ...
      # Changeable Start
      items = Item.all.where('name LIKE ?', '%Powerball%')
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
      most_popular_items = Item.joins(:order_items).select('items.*, count(order_items.item_id)').group('order_items.item_id').order('count(order_items.item_id) DESC').limit(2)
      # Changeable Stop
